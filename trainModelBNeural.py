...
def preprocess_race_data(df):
    ...
    # Assuming df_full has 7 rows, and you want 14
    dmy = {
        # runner
        "results": [],
        #"place": place,
        ...
        "age": 0,
        "AGEMISDATA": 1,
        "drawBiasScore": 0, # update to use group avg
        "goingBias": 0,
        "raceCount": 0
    }
    ...
    while len(df) < 14:
        df = pd.concat([df, pd.DataFrame([dmy])], ignore_index=True)
    return df
...
def create_default_runner(race_meta, place):
    ...
    # Check for any NaN values in the runner dictionary
    if any(pd.isna(value) if not isinstance(value, (list, np.ndarray)) else False for value in runner.values()):
        print("[Warning] Found NaN in runner:", runner)
...
def pad_race_features(group, race_meta, feature_columns):
    ...
    # Fill NaNs in speedRating with group mean
    if "speedRating" in group.columns:
        if group["speedRating"].isnull().any():
            mean_speed = group["speedRating"].mean(skipna=True)
            group.loc[group["speedRating"].isnull(), "speedRating"] = mean_speed

    # If there is a dummy runner (horseId == "dummyID"), fill in race_meta
    if "horseId" in group.columns:
        dummy_mask = group["horseId"] == "dummyID"
        if dummy_mask.any():
            for field in [
                "date", "race", "venue", "racetrack", "going", 
                "class", "racecourse", "distance", "money"
            ]:
                if field in race_meta:
                    group.loc[dummy_mask, field] = race_meta[field]

    # Ensure all feature columns exist
    for col in feature_columns:
        if col not in group.columns:
            group[col] = 0.0  # Add missing feature with neutral value
...
def soft_label(row):
    ...
    if row['place'] == 1:
        return 1.0
    elif row['place'] in [2, 3, 4]:
        return max(0.0, 1.0 - (row['lbw'] / avg_lbw))
    else:
        return 0.0

def process_by_race(df, features_list=None):
    X, y, feature_columns = [], [], []
    ...
    for (date, race_num), group in df.groupby(["date", "race"]):
        ...
        #print("group (head):\n", group.head()) # Print the first few rows
        race_meta = group.iloc[0]  # shared race-level features
        feature_columns = [col for col in df.columns if col not in ['place', 'lbw'] + DROP_COLUMNS] if features_list is None else features_list
...
def save_model(model, r2, training_variables, csvPath, optimizer=None):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_folder = f"models/neural_net_model_{timestamp}"
    os.makedirs(model_folder, exist_ok=True)

    # Save model state_dict (weights and biases)
    ...
    metadata = {
        ...
        "architecture": "Simple 3-layer NN",
        "epochs": 100,
        "optimizer": "Adam",
        ...
        "metric_type": "R2",
        "csvTrained": csvPath,
        "input_dim": len(training_variables),  # Save input dimension based on feature count
    }

    # Save optimizer state_dict if available
    if optimizer is not None:
        optimizer_path = os.path.join(model_folder, "optimizer.pth")
        torch.save(optimizer.state_dict(), optimizer_path)
        metadata["optimizer_state"] = "saved"
    else:
        metadata["optimizer_state"] = "not_saved"
