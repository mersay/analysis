...
def predictOldRaces():
    ...
    if isinstance(obj, dict):
        ...
        for k in obj.keys():
            print(k)  # Shows layer names like 'fc1.weight'
    ...
    w_model, w_odds = compute_weights(df)
...
def predict():
    ...
    # Prepare data
    x_cat_tensor, x_num_tensor, y_tensor, isDummyTensor = prepare_data_for_races(grouped_races, race_list, config)
    ...
    for i in range(predicted_lbws.shape[0]):
        ...
        if len(true_valid) > 1:
            race_r2 = r2_score(true_valid.numpy(), pred_valid.numpy())
            print(f"  R² for race {i + 1}: {race_r2:.4f}")
        else:
            print("  Not enough data to compute R² for this race.")
    df["logits"] = all_preds
    # Global R²
