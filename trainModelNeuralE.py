...
# just like D, but using lbw as Y
...
class RaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return (
            {k: v[idx] for k, v in self.x_cat.items()},  # x_cat
            self.x_num[idx],                            # x_num
            self.y[idx]                                 # y
        )
...
class WinnerPredictor(nn.Module):
    ...
    def forward(self, x_cat, x_num):
        # x_cat and x_num should be [batch_size, num_runners, feature_dim]
        batch_size, num_runners, _ = x_num.shape

        embedded = [
            self.embeddings[col](x_cat[col]) for col in sorted(x_cat.keys())
        ]  # each [batch_size, num_runners, embedding_dim]

        embedded = torch.cat(embedded, dim=2)  # [batch_size, num_runners, total_embed_dim]
        x = torch.cat([embedded, x_num], dim=2)  # [batch_size, num_runners, total_input_dim]

        logits = self.mlp(x).squeeze(-1)  # [batch_size, num_runners]
        return logits
...
def prepare_data_for_races(races, race_list, config, is_future=False):
    ...
    for race_key in race_list:
        ...
        print("race_date", race_date, "race_number", race_number)
        ...
        for col in avg_columns:
            if col in race_df.columns:
                valid_vals = race_df[race_df[col] != -1][col]
                race_avg_values[col] = valid_vals.mean() if not valid_vals.empty else -1
        
        # Step 2: Create padded numeric features list — dummy runners first
        padded_num_list = []
        for _ in range(max_runners - num_current_runners):
            dummy_vals = []
            for col in numerical_cols_to_scale:
                val = race_avg_values.get(col, dummy_runner.get(col, -1.0)) \
                    if col in avg_columns else dummy_runner.get(col, -1.0)
                dummy_vals.append(val)
            dummy_vals.append(1)  # isDummy = 1
            padded_num_list.append(dummy_vals)
    
        # Step 3: Process real runners — apply race averages and scaling
        for col in avg_columns:
            if col in race_df.columns:
        ...
        for _ in range(max_runners - num_current_runners):
            for col in config["categorical_columns"]:
                if col in race_shared_fields:
                    padded_cat_dict[col].append(shared_values.get(col, "Unknown"))
                else:
                    padded_cat_dict[col].append(dummy_runner.get(col, "Unknown"))
            padded_cat_dict["isDummy"].append(1)

        # Now process real runners
        ...
        # Always include this:
        x_cat_encoded_padded_race["isDummy"] = torch.tensor(padded_cat_dict["isDummy"], dtype=torch.long)

        x_cat_padded_all_races.append(x_cat_encoded_padded_race)
        x_num_padded_all_races.append(x_num_padded_race)

        # Step 6: Prepare y if not in future mode
        if not is_future:
            lbw_values = [99.0] * (max_runners - len(race_df))
            lbw_values += race_df[config["target_variable"]].tolist()
            y.append(lbw_values)
    ...
    is_dummy_tensor = torch.stack([race_data["isDummy"] for race_data in x_cat_padded_all_races])

    if not is_future:
        for i, row in enumerate(y):
            for j, val in enumerate(row):
                if not isinstance(val, (int, float)):
                    print(f"Non-numeric value found at y[{i}][{j}]: {val} ({type(val)})")
        ...
        return x_cat_train_tensor, x_num_train_tensor, is_dummy_tensor


def masked_mse_loss(pred, target, mask_val=99.0):
...
def train_model(df, config):
    ...
    x_cat_train, x_num_train, y_train, isDummyTensor = prepare_data_for_races(races, train_races, config)
    x_cat_val, x_num_val, y_val, is_dummy_tensor = prepare_data_for_races(races, val_races, config)

    # Check for NaN or infinite values in your data
    assert not torch.isnan(x_num_train).any(), "NaN found in numerical features"
    assert not torch.isinf(x_num_train).any(), "Infinite values found in numerical features"
    assert not torch.isnan(y_train).any(), "NaN found in target variable"
    assert not torch.isinf(y_train).any(), "Infinite values found in target variable"

    train_dataset = RaceDataset(x_cat_train, x_num_train, y_train)
    val_dataset = RaceDataset(x_cat_val, x_num_val, y_val)
    ...
    for epoch in range(100):
        ...
        for x_cat_batch, x_num_batch, y_batch in train_loader:
            optimizer.zero_grad()
            logits = model(x_cat_batch, x_num_batch)  # [batch_size, num_runners]
            loss = loss_fn(logits, y_batch)  # use masked loss
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for i in range(len(y_val_tensor)):
                ...
                y_batch = y_val_tensor[i]  # shape: [num_runners]

                output = model(x_cat_batch, x_num_batch).squeeze(0)  # shape: [num_runners]

                mask = y_batch != 99.0  # only consider real runners
                loss = nn.functional.mse_loss(output[mask], y_batch[mask])
                val_loss += loss.item()

        val_loss /= len(y_val_tensor)
        print(f"Epoch {epoch:03d} | Train Loss: {total_loss:.4f} | Val MSE: {val_loss:.4f}")
    # Save the trained model (example with PyTorch)
...
if __name__ == "__main__":
    ...
    dummy_runner = {
        ...
        "speedRating": 0.0,
        "SRMISDATA": 1,
        "daysSinceLastRace": 4.0,
        "daysSinceLastRaceJockey": 4,
        "jocMisData": 1,
        "jocWinPercent": 0.0,
        "jocPlacePercent": 0,
        "trainerMisData": 1,
        "trainerWinPercent": 0.0,
        "trainerPlacePercent": 0,
        "avglbw": 0.0,
        "age": 4.0,
        "AGEMISDATA": 1,
        "goingBias": 0.0,
